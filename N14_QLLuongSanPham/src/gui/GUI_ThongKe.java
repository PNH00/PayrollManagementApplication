...
//	Màn hình thống kê giới tính nhân viên
	...
	private JButton btnLuongTatCaCacThang;
...
//	Khởi tạo màn hình thống kê
	...
	public GUI_ThongKe() {
		...
		btnLuongCN.setBounds(342, 84, 151, 167);
		...
		comboBoxCN.setBounds(438, 254, 43, 27);
		...
		comboBoxLuong.setBounds(268, 254, 43, 27);
		...
		lblNewLabel_4_2.setBounds(181, 252, 87, 31);
		...
		lblNewLabel_2_1_1.setBounds(10, 61, 151, 27);
		panelThongKeLuong.add(lblNewLabel_2_1_1);
		
		JLabel lblNewLabel_2_1_1_1 = new JLabel("ĐỘ TƯƠNG QUAN");
		lblNewLabel_2_1_1_1.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel_2_1_1_1.setFont(new Font("Times New Roman", Font.PLAIN, 15));
		lblNewLabel_2_1_1_1.setBounds(171, 58, 151, 31);
		...
		lblNewLabel_2_1_1_2.setBounds(342, 61, 151, 27);
		panelThongKeLuong.add(lblNewLabel_2_1_1_2);
		
		btnLuongTatCaCacThang = new JButton("New button");
		btnLuongTatCaCacThang.setBackground(Color.WHITE);
		btnLuongTatCaCacThang.setBounds(504, 84, 151, 197);
		btnLuongTatCaCacThang.setIcon(new ImageIcon("data\\\\img\\\\last.png"));
		panelThongKeLuong.add(btnLuongTatCaCacThang);
		
		JLabel lblNewLabel_2_1_1_2_1 = new JLabel("TẤT CẢ CÁC THÁNG");
		...
		lblNewLabel_2_1_1_2_1.setBounds(504, 61, 151, 27);
		panelThongKeLuong.add(lblNewLabel_2_1_1_2_1);
...
//		Kết nối cơ sở dữ liệu và khởi tạo đối tượng DAO
//		 try{
...
//		Màn hình thống kê tình trạng thanh toán
...
//		
////		Màn hình thống kê tương quan của nhân viên và công nhân
...
//		
////		Màn hình thống kê lương tất cả các tháng		
//		jframeLuongTatCaCacThang = new JFrame();
//		jframeLuongTatCaCacThang.setSize(900,700);
//		jframeLuongTatCaCacThang.setLocationRelativeTo(null);
//		JFreeChart jFreeChartLuongTrongNam = createChartLuongTrongNam(phieuLuongCongNhan_DAO.getAllPhieuLuongCongNhan(),phieuLuongNhanVien_DAO.getAllPhieuLuongNhanVien());
//		ChartPanel chartPanelLuongTrongNam = new ChartPanel(jFreeChartLuongTrongNam);
//		chartPanelLuongTrongNam.setPreferredSize(new java.awt.Dimension(560, 367));
//		jframeLuongTatCaCacThang.getContentPane().add(chartPanelLuongTrongNam);
		...
		btnLuongTatCaCacThang.addActionListener(this);
		
	}
	
//	Hàm khởi tạo biểu đồ giá trị hợp đồng
...
//	 Hàm khởi tạo bộ dữ liệu để thống kê giá trị hợp đồng
...
//		Hàm khởi tạo biểu đồ lương trong năm
		public JFreeChart createChartLuongTrongNam(List<PhieuLuongCongNhan> listLuongCN, List<PhieuLuongNhanVien> listLuongNV ) {
			 CategoryDataset categoryDataset = createDatasetLuongTrongNam();
			 double tongGiaTri = 0;
			 for(PhieuLuongCongNhan phieuLuongCongNhan : listLuongCN) {
				 tongGiaTri+=phieuLuongCongNhan.getThanhTien();
				 }
			 for(PhieuLuongNhanVien phieuLuongNhanVien : listLuongNV) {
				 tongGiaTri+=phieuLuongNhanVien.getThanhTien();
				 }
			 JFreeChart barChart = ChartFactory.createBarChart(
		                "Tổng lương đã chi trong năm: "+NumberFormat.getCurrencyInstance(new Locale("vi","VN")).format(tongGiaTri),
		                "Tháng", "Thành tiền",
		                categoryDataset, PlotOrientation.VERTICAL, false, false, false);
			 return barChart;
		    }
//		 Hàm khởi tạo bộ dữ liệu để thống kê lương trong năm
		 @SuppressWarnings("deprecation")
		public CategoryDataset createDatasetLuongTrongNam() {
			 ...
			 for(PhieuLuongCongNhan phieuLuongCongNhan : phieuLuongCongNhan_DAO.getAllPhieuLuongCongNhan()) {
				 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==1 ) {
						mot+=phieuLuongCongNhan.getThanhTien();
					 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==2) {
							hai+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==3 ) {
							ba+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==4) {
							bon+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==5) {
							nam+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==6) {
							sau+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==7) {
							bay+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==8) {
							tam+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==9) {
							chin+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==10) {
							muoi+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==11) {
							muoiMot+=phieuLuongCongNhan.getThanhTien();
						 }
					 if(phieuLuongCongNhan.getNgayTao().getMonth()+1==12) {
			 ...
			 for (PhieuLuongNhanVien  phieuLuongNhanVien : phieuLuongNhanVien_DAO.getAllPhieuLuongNhanVien()) {
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==1) {
						mot+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==2) {
						hai+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==3) {
						ba+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==4) {
						bon+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==5) {
						nam+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==6) {
						sau+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==7) {
						bay+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==8) {
						tam+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==9) {
						chin+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==10) {
						muoi+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==11) {
						muoiMot+= phieuLuongNhanVien.getThanhTien();
					 }
				 if(phieuLuongNhanVien.getNgayTao().getMonth()+1==12) {
...
//		Hàm khởi tạo biểu đồ tình trạng thanh toán hợp đồng
...
//	 Hàm khởi tạo bộ dữ liệu để thống kê tình trạng thanh toán hợp đồng
...
//		Hàm khởi tạo biểu đồ thống kê độ tuổi công nhân
...
//	 Hàm khởi tạo bộ dữ liệu để thống kê độ tuổi công nhân
	@SuppressWarnings("deprecation")
...
//	Hàm khỏi tạo biểu đồ thống kê nhân sự
...
//	Hàm khởi tạo bộ dữ liệu để thống kê nhân sự
...
//	Hàm khởi tạo biểu đồ thống kê độ tuổi nhân viên
...
//	Hàm khởi tạo bộ dữ liệu để thông kê tuổi nhân viên 
	@SuppressWarnings("deprecation")
...
//	Hàm tạo biểu đồ thống kê lương nhân viên
	@SuppressWarnings("deprecation")
...
//	Hàm khởi tạo bộ dữ liệu thống kê lương nhân viên
	@SuppressWarnings("deprecation")
...
//	Hàm tạo biểu đồ thống kê lương công nhân
	@SuppressWarnings("deprecation")
...
//	Hàm khởi tạo bộ dữ liệu thống kê lương công nhân
	@SuppressWarnings("deprecation")
...
//	Hàm khởi tạo biểu đồ thống kê lương 
	@SuppressWarnings("deprecation")
...
//	Hàm khởi tạo bộ dữ liệu thống kê lương
	@SuppressWarnings("deprecation")
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnLuongTatCaCacThang)) {
			jframeLuongTatCaCacThang.setVisible(true);
		}
...
//	Hàm hiển thị màn hình thống kê lương nhân viên
...
//	Hàm hiển thị màn hình thống kê lương công nhân
...
//	Hàm hiển thị màn hình thống kê lương 
